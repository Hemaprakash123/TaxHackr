...
const capitalGainsDataMock = {
  capitalGains: {
    stcg: { profits: 100, losses: 500 }, // As per example
    ltcg: { profits: 1200, losses: 100 }, // As per example
  }
};
...
export const fetchHoldings = () => {
  return new Promise((resolve) => {
    setTimeout(() => {
      // Ensure unique ID for each holding, useful for React keys and selection
      resolve(holdingsDataMock.map(h => ({ ...h, id: h.id || (h.coin + Math.random().toString(16).slice(2)) })));
    }, 500);
  });
};
